...
const STATUS_ACTION_META: Partial<
  Record<AppointmentStatus, StatusActionMeta>
> = {
  CONFIRMED: {
    label: "Confirm",
...
export function BookingsTable({ appointments }: BookingsTableProps) {
          ...
          <TableBody>
            {appointments.map((appointment) => {
              ...
              const actionEntries = Object.entries(STATUS_ACTION_META).filter(
                ([status]) =>
                  canTransition(appointment.status, status as AppointmentStatus)
              )
